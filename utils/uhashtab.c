...
int hashtab_remove(struct hashtab *h, unsigned char *key)
{
	...
	if (cur && (h->keycmp(h, key, cur->key) != 0))
    {
        ...
        char humankey[33];
        MD5Human(cur->key, humankey);
        printf("doesnt exist cur->key %s\n", humankey);
        MD5Human(key, humankey);
        printf("doesnt exist key %s\n", humankey);
#endif
        return -ENOENT;		//no such entry
    }

	/* We want to unlink or remove cur from this hashtable */
	if (prev)
	{
		/* Not the first entry itself */
		prev->next = cur->next;
...
void *hashtab_search(struct hashtab *h, unsigned char *key)
{
	...
	if (h == NULL || key == NULL)
	{
		fprintf(stdout, "Problem in %s, maybe hash-table not inited?\n",
				__FUNCTION__);
	}
...
void hashtab_destroy(struct hashtab *h)
{
	...
	while (cur) {
		temp = cur;
		cur = cur->next;
		if (h->rementry)	//if this delete function defined, use it...
		{
			h->rementry((void*)temp->datum);
		}
